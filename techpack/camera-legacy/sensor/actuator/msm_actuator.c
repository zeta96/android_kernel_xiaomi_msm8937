							...
							int config)
{
	...
	for (i = 0; i < cnt; i++) {
		if (a_ctrl->act_device_type == MSM_CAMERA_PLATFORM_DEVICE) {
			rc = legacy_msm_camera_config_single_vreg(&(a_ctrl->pdev->dev),
				&vreg_cfg->cam_vreg[i],
				(struct regulator **)&vreg_cfg->data[i],
				config);
		} else if (a_ctrl->act_device_type ==
			MSM_CAMERA_I2C_DEVICE) {
			rc = legacy_msm_camera_config_single_vreg(
...
static struct msm_camera_i2c_fn_t msm_sensor_qup_func_tbl = {
	...
	.i2c_poll = legacy_msm_camera_qup_i2c_poll,
};
	...
	const struct i2c_device_id *id)
{
	...
	if (of_find_property(client->dev.of_node,
		...
		rc = legacy_msm_camera_get_dt_vreg_data(client->dev.of_node,
	...
	legacy_msm_sd_register(&act_ctrl_t->msm_sd);
	legacy_msm_cam_copy_v4l2_subdev_fops(&msm_actuator_v4l2_subdev_fops);
#ifdef CONFIG_COMPAT
...
static int32_t msm_actuator_platform_probe(struct platform_device *pdev)
{
	...
	rc = legacy_msm_sensor_driver_get_gpio_data(&(msm_actuator_t->gconf),
		(&pdev->dev)->of_node);
	if (rc <= 0) {
	...
	legacy_msm_sd_register(&msm_actuator_t->msm_sd);
