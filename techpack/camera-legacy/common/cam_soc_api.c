...
struct msm_cam_bus_pscale_data legacy_g_cv[CAM_BUS_CLIENT_MAX];
...
int legacy_msm_camera_i2c_dev_get_clk_info(struct device *dev,
			struct msm_cam_clk_info **clk_info,
			struct clk ***clk_ptr,
			size_t *num_clk)
{
...
EXPORT_SYMBOL(legacy_msm_camera_i2c_dev_get_clk_info);

/* Get all clocks from DT  for platform devices */
int legacy_msm_camera_get_clk_info(struct platform_device *pdev,
			struct msm_cam_clk_info **clk_info,
			struct clk ***clk_ptr,
			size_t *num_clk)
{
...
EXPORT_SYMBOL(legacy_msm_camera_get_clk_info);

/* Get all clocks and multiple rates from DT */
int legacy_msm_camera_get_clk_info_and_rates(
...
EXPORT_SYMBOL(legacy_msm_camera_get_clk_info_and_rates);

/* Enable/Disable all clocks */
int legacy_msm_camera_clk_enable(struct device *dev,
		struct msm_cam_clk_info *clk_info,
...
EXPORT_SYMBOL(legacy_msm_camera_clk_enable);

/* Set rate on a specific clock */
long legacy_msm_camera_clk_set_rate(struct device *dev,
			struct clk *clk,
			long clk_rate)
{
	int rc = 0;
...
EXPORT_SYMBOL(legacy_msm_camera_clk_set_rate);
...
int legacy_msm_camera_i2c_dev_put_clk_info(struct device *dev,
				struct msm_cam_clk_info **clk_info,
				struct clk ***clk_ptr, int cnt)
{
	int rc = 0;
...
EXPORT_SYMBOL(legacy_msm_camera_i2c_dev_put_clk_info);

/* release memory allocated for clocks for platform devices */
int legacy_msm_camera_put_clk_info(struct platform_device *pdev,
				struct msm_cam_clk_info **clk_info,
				struct clk ***clk_ptr, int cnt)
{
	int rc = 0;
...
EXPORT_SYMBOL(legacy_msm_camera_put_clk_info);

int legacy_msm_camera_put_clk_info_and_rates(struct platform_device *pdev,
		struct msm_cam_clk_info **clk_info,
...
EXPORT_SYMBOL(legacy_msm_camera_put_clk_info_and_rates);

/* Get reset info from DT */
int legacy_msm_camera_get_reset_info(struct platform_device *pdev,
...
int legacy_msm_camera_get_regulator_info(struct platform_device *pdev,
...
int legacy_msm_camera_regulator_enable(struct msm_cam_regulator *vdd_info,
				int cnt, int enable)
{
	int i;
	int rc;
...
EXPORT_SYMBOL(legacy_msm_camera_regulator_enable);

/* set regulator mode */
int legacy_msm_camera_regulator_set_mode(struct msm_cam_regulator *vdd_info,
				int cnt, bool mode)
{
	int i;
	int rc;
...
EXPORT_SYMBOL(legacy_msm_camera_regulator_set_mode);

/* Put regulators regulators */
void legacy_msm_camera_put_regulators(struct platform_device *pdev,
...
struct resource *legacy_msm_camera_get_irq(struct platform_device *pdev,
							char *irq_name)
{
	if (!pdev || !irq_name) {
		pr_err("Invalid params\n");
...
EXPORT_SYMBOL(legacy_msm_camera_get_irq);

int legacy_msm_camera_register_irq(struct platform_device *pdev,
...
EXPORT_SYMBOL(legacy_msm_camera_register_irq);

int legacy_msm_camera_register_threaded_irq(struct platform_device *pdev,
...
EXPORT_SYMBOL(legacy_msm_camera_register_threaded_irq);

int legacy_msm_camera_enable_irq(struct resource *irq, int enable)
{
	if (!irq) {
		pr_err("Invalid params\n");
		return -EINVAL;
...
EXPORT_SYMBOL(legacy_msm_camera_enable_irq);

int legacy_msm_camera_unregister_irq(struct platform_device *pdev,
...
EXPORT_SYMBOL(legacy_msm_camera_unregister_irq);

void __iomem *legacy_msm_camera_get_reg_base(struct platform_device *pdev,
...
EXPORT_SYMBOL(legacy_msm_camera_get_reg_base);

uint32_t legacy_msm_camera_get_res_size(struct platform_device *pdev,
	char *device_name)
{
	struct resource *mem;
...
EXPORT_SYMBOL(legacy_msm_camera_get_res_size);


int legacy_msm_camera_put_reg_base(struct platform_device *pdev,
...
EXPORT_SYMBOL(legacy_msm_camera_put_reg_base);

/* Register the bus client */
uint32_t legacy_msm_camera_register_bus_client(struct platform_device *pdev,
	enum cam_bus_client id)
{
	...
	legacy_g_cv[id].pdata = pdata;
	legacy_g_cv[id].bus_client = bus_client;
	legacy_g_cv[id].vector_index = 0;
	legacy_g_cv[id].num_usecases = num_usecases;
	legacy_g_cv[id].num_paths = num_paths;
	mutex_init(&legacy_g_cv[id].lock);
	CDBG("Exit Client ID: %d\n", id);
	return 0;
}
EXPORT_SYMBOL(legacy_msm_camera_register_bus_client);

/* Update the bus bandwidth */
uint32_t legacy_msm_camera_update_bus_bw(int id, uint64_t ab, uint64_t ib)
{
	...
	mutex_lock(&legacy_g_cv[id].lock);
	idx = legacy_g_cv[id].vector_index;
	idx = 1 - idx;
	legacy_g_cv[id].vector_index = idx;
	mutex_unlock(&legacy_g_cv[id].lock);

	pdata = legacy_g_cv[id].pdata;
	...
	CDBG("Register client ID : %d [ab : %llx, ib : %llx], update :%d\n",
		id, ab, ib, idx);
	msm_bus_scale_client_update_request(legacy_g_cv[id].bus_client, idx);

	return 0;
}
EXPORT_SYMBOL(legacy_msm_camera_update_bus_bw);
	...
	int vector_index)
{
	...
	if (vector_index < 0 || vector_index > legacy_g_cv[id].num_usecases) {
		pr_err("Invalid params");
		return -EINVAL;
	}

	CDBG("Register client ID : %d vector idx: %d,\n", id, vector_index);
	msm_bus_scale_client_update_request(legacy_g_cv[id].bus_client,
		vector_index);

	return 0;
}
EXPORT_SYMBOL(legacy_msm_camera_update_bus_vector);

/* Unregister the bus client */
uint32_t legacy_msm_camera_unregister_bus_client(enum cam_bus_client id)
{
	if (id >= CAM_BUS_CLIENT_MAX) {
		pr_err("Invalid params");
		return -EINVAL;
	...
	mutex_destroy(&legacy_g_cv[id].lock);
	msm_bus_scale_unregister_client(legacy_g_cv[id].bus_client);
	legacy_g_cv[id].bus_client = 0;
...
EXPORT_SYMBOL(legacy_msm_camera_unregister_bus_client);
