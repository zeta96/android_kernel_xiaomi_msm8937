...
/*
 * legacy_msm_fd_buf_init - vb2_ops buf_init callback.
 * @vb: Pointer to vb2 buffer struct.
 */
int legacy_msm_fd_buf_init(struct vb2_buffer *vb)
{
...
static void msm_fd_buf_queue(struct vb2_buffer *vb)
{
	...
	legacy_msm_fd_hw_add_buffer(ctx->fd_device, fd_buffer);

	if (vb->vb2_queue->streaming)
		legacy_msm_fd_hw_schedule_and_start(ctx->fd_device);
...
static int msm_fd_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	ret = legacy_msm_fd_hw_get(ctx->fd_device, ctx->settings.speed);
	if (ret < 0) {
		dev_err(ctx->fd_device->dev, "Can not acquire fd hw\n");
		goto out;
	}

	ret = legacy_msm_fd_hw_schedule_and_start(ctx->fd_device);
	if (ret < 0)
...
static void msm_fd_stop_streaming(struct vb2_queue *q)
{
	...
	legacy_msm_fd_hw_remove_buffers_from_queue(ctx->fd_device, q);
	legacy_msm_fd_hw_put(ctx->fd_device);
...
static struct vb2_ops msm_fd_vb2_q_ops = {
	...
	.buf_init        = legacy_msm_fd_buf_init,
	...
	enum dma_data_direction dma_dir)
{
	...
	ret = legacy_msm_fd_hw_map_buffer(pool, vaddr, buf);
...
static void msm_fd_put_userptr(void *buf_priv)
{
	...
	legacy_msm_fd_hw_unmap_buffer(buf_priv);
...
static int msm_fd_vbif_error_handler(void *handle, uint32_t error)
{
	...
	if (error == CPP_VBIF_ERROR_HANG) {
		...
		/* Halt and reset */
		legacy_msm_fd_hw_put(fd);
		legacy_msm_fd_hw_get(fd, ctx->settings.speed);

		/* Get active buffer */
		active_buf = legacy_msm_fd_hw_get_active_buffer(fd);
		...
		legacy_msm_fd_hw_add_buffer(fd, active_buf);

		/* Schedule and restart */
		ret = legacy_msm_fd_hw_schedule_next_buffer(fd);
		if (ret) {
...
static int msm_fd_open(struct file *file)
{
	...
	ret = legacy_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_FD,
			CAM_AHB_SVS_VOTE);
	if (ret < 0) {
	...
	legacy_msm_cpp_vbif_register_error_handler((void *)ctx,
...
static int msm_fd_release(struct file *file)
{
	...
	if (ctx->work_buf.fd != -1)
		legacy_msm_fd_hw_unmap_buffer(&ctx->work_buf);
	...
	if (legacy_cam_config_ahb_clk(NULL, 0, CAM_AHB_CLIENT_FD,
...
static int msm_fd_s_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	case V4L2_CID_FD_WORK_MEMORY_FD:
		...
		if (ctx->work_buf.fd != -1)
			legacy_msm_fd_hw_unmap_buffer(&ctx->work_buf);
		if (a->value >= 0) {
			ret = legacy_msm_fd_hw_map_buffer(&ctx->mem_pool,
	...
	struct msm_fd_face_data *face, int idx)
{
	...
	legacy_msm_fd_hw_get_result_angle_pose(fd, idx, &face->angle, &face->pose);

	legacy_msm_fd_hw_get_result_conf_size(fd, idx, &face->confidence,
		&face->face.width);
	face->face.height = face->face.width;

	face->face.left = legacy_msm_fd_hw_get_result_x(fd, idx);
	face->face.top = legacy_msm_fd_hw_get_result_y(fd, idx);
...
static void msm_fd_wq_handler(struct work_struct *work)
{
	...
	stats->face_cnt = legacy_msm_fd_hw_get_face_count(fd);
	...
	legacy_msm_fd_hw_schedule_next_buffer(fd);
	...
	legacy_msm_fd_hw_buffer_done(fd, active_buf);
}
...
static int fd_probe(struct platform_device *pdev)
{
	...
	/* Get resources */
	ret = legacy_msm_fd_hw_get_mem_resources(pdev, fd);
	if (ret < 0) {
	...
	ret = legacy_msm_camera_get_regulator_info(pdev, &fd->vdd_info,
		&fd->num_reg);
	if (ret < 0) {
		dev_err(&pdev->dev, "Fail to get regulators\n");
		goto error_get_regulator;
	}
	ret = legacy_msm_camera_get_clk_info_and_rates(pdev, &fd->clk_info,
	...
	ret = legacy_msm_camera_register_bus_client(pdev, CAM_BUS_CLIENT_FD);
	if (ret < 0) {
	...
	ret = legacy_msm_fd_hw_get(fd, 0);
	if (ret < 0) {
		dev_err(&pdev->dev, "Fail to get hw\n");
		goto error_hw_get_request_irq;
	}
	fd->hw_revision = legacy_msm_fd_hw_get_revision(fd);

	legacy_msm_fd_hw_put(fd);

	ret = legacy_msm_fd_hw_request_irq(pdev, fd, msm_fd_wq_handler);
	if (ret < 0) {
...
error_v4l2_register:
	legacy_msm_fd_hw_release_irq(fd);
error_hw_get_request_irq:
	legacy_msm_camera_unregister_bus_client(CAM_BUS_CLIENT_FD);
error_get_bus:
	legacy_msm_camera_put_clk_info_and_rates(pdev, &fd->clk_info,
		&fd->clk, &fd->clk_rates, fd->clk_rates_num, fd->clk_num);
error_get_clocks:
	legacy_msm_camera_put_regulators(pdev, &fd->vdd_info, fd->num_reg);
error_get_regulator:
	legacy_msm_fd_hw_release_mem_resources(fd);
error_mem_resources:
	kfree(fd);
	return ret;
}
